-- Ensure AVM provider run-log tables exist and add HouseCanary run logging.
...
CREATE TABLE IF NOT EXISTS housecanary_runs (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  run_id UUID NOT NULL UNIQUE,
  application_id UUID,
  created_at TIMESTAMPTZ DEFAULT NOW(),
...
CREATE INDEX IF NOT EXISTS idx_housecanary_runs_status ON housecanary_runs (status);
CREATE INDEX IF NOT EXISTS idx_housecanary_runs_product ON housecanary_runs (product);

ALTER TABLE housecanary_runs ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Service role full access on housecanary_runs" ON housecanary_runs;
CREATE POLICY "Service role full access on housecanary_runs"
  ON housecanary_runs FOR ALL
  USING (TRUE)
  WITH CHECK (TRUE);
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'cleanup-old-housecanary-runs') THEN
    PERFORM cron.schedule(
      'cleanup-old-housecanary-runs',
      '40 2 * * *',
      $$DELETE FROM housecanary_runs WHERE created_at < NOW() - INTERVAL '30 days'$$
    );
  END IF;
END
$$;
