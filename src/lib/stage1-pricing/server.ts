...
export function computeStage1Pricing(request: Stage1PricingRequest): Stage1PricingResponse {
  ...
  const chooseBestXSummary = (eligibility: Stage1Eligibility, fallbackQuote: Stage1ExecutionQuote, requestedRates: number[], getQuote: (rateOverride?: number) => Stage1ExecutionQuote, maxPrice: number): InvestorSummary => {
    ...
    const borrowerVisibleCandidates = usableCandidates.filter(summary => {
      const { pointsLabel, pointsValue } = getBorrowerFacingPoints(summary.quote.purchasePrice);
      return pointsLabel === 'Rebate' || pointsValue <= 3;
    });
    const rankedCandidates = (borrowerVisibleCandidates.length > 0 ? borrowerVisibleCandidates : usableCandidates).sort((a, b) => {
      const aPoints = getBorrowerFacingPoints(a.quote.purchasePrice);
      const bPoints = getBorrowerFacingPoints(b.quote.purchasePrice);
      return aPoints.pointsValue - bPoints.pointsValue
        || Math.abs(aPoints.rawDeltaFromPar) - Math.abs(bPoints.rawDeltaFromPar)
        || b.quote.rate - a.quote.rate
        || a.investor.localeCompare(b.investor);
    });
