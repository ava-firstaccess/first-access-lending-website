...
import type { InvestorPriceLadderRow, InvestorSummary, OsbLockPeriod, PricingViewEngine, RatesheetDateInfo, Stage1Eligibility, Stage1ExecutionQuote, Stage1PricingEngineResult, Stage1PricingRequest, Stage1PricingResponse, TesterInput, VerusDrawPeriodYears, VerusLockPeriodDays } from './types';
...
function getActiveResult(engine: PricingViewEngine, input: TesterInput, effectiveTargetPrice: number, effectiveManualRateOverride: number | undefined, tolerance: number, hasTargetPriceOverride: boolean): Stage1PricingEngineResult | null {
  ...
  const ratesheetDates = getStage1RatesheetDateMap();
  const withGuide = (result: Omit<Stage1PricingEngineResult, 'guideMaxPrice' | 'ratesheetInfo'>): Stage1PricingEngineResult => ({ ...result, guideMaxPrice: getLadderMaxBuyPrice(result.quote, input, result.maxPrice), ratesheetInfo: ratesheetDates[result.quote.engine] ?? null });
  if (engine === 'Button') {
