...
export function computeStage1Pricing(request: Stage1PricingRequest): Stage1PricingResponse {
  ...
  const makeSummary = (eligibility: Stage1Eligibility, quote: Stage1ExecutionQuote): InvestorSummary => { const discountPoints = roundUpToEighth(100 - quote.purchasePrice); const buyPrice = Number(quote.purchasePrice.toFixed(3)); const deltaFromTarget = roundToThree(quote.purchasePrice - defaultBackendTargetPrice); const windowMatched = eligibility.eligible && quote.purchasePrice >= bestExWindowLowerBound && quote.purchasePrice <= bestExWindowUpperBound; return { investor: quote.engine, eligibility, quote, discountPoints, buyPrice, windowMatched, deltaFromTarget }; };
