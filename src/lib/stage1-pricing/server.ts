...
function getLadderMaxBuyPrice(quote: Stage1ExecutionQuote, selectedLoanAmount: number, fallbackMaxPrice: number) {
  if (quote.engine === 'NewRez') return 106;
  if (quote.engine === 'OSB') return 106;
  if (quote.engine === 'Button') return 105;
  if (quote.engine === 'Vista') return 105;
  if (quote.engine === 'Verus') return 104;
  if (quote.engine === 'Deephaven') {
    if (quote.program === 'Expanded Prime') return selectedLoanAmount > 500000 ? 104 : 105;
    return 105;
  }
  return fallbackMaxPrice > 0 ? Math.floor(fallbackMaxPrice) : 0;
}
...
export function computeStage1Pricing(request: Stage1PricingRequest): Stage1PricingResponse {
  ...
  const makeSummary = (eligibility: Stage1Eligibility, quote: Stage1ExecutionQuote, maxPrice: number, requestedRates: number[], getQuoteForRate: (rateOverride?: number) => Stage1ExecutionQuote): InvestorSummary => { const overlaidEligibility = applyAvmOverlay(eligibility, quote.engine, input); const selectionTarget = getBestXSelectionTarget(effectiveTargetPrice, maxPrice, hasTargetPriceOverride); const bestExWindowLowerBound = roundToThree(selectionTarget - BEST_EX_WINDOW_FLOOR); const bestExWindowUpperBound = roundToThree(selectionTarget + BEST_EX_WINDOW_CEILING); const displayPrice = getBorrowerFacingDisplayPrice(quote.purchasePrice, effectiveTargetPrice, displayTargetPrice); const { pointsValue: discountPoints } = getBorrowerFacingPoints(displayPrice, displayTargetPrice); const buyPrice = 0; const deltaFromTarget = roundToThree(quote.purchasePrice - effectiveTargetPrice); const ladderMaxBuyPrice = getLadderMaxBuyPrice(quote, input.desiredLoanAmount ?? 0, maxPrice); const respectsMaxPrice = ladderMaxBuyPrice <= 0 || quote.purchasePrice <= ladderMaxBuyPrice + 0.0001; const windowMatched = overlaidEligibility.eligible && respectsMaxPrice && quote.purchasePrice >= bestExWindowLowerBound && quote.purchasePrice <= bestExWindowUpperBound; return { investor: quote.engine, eligibility: overlaidEligibility, quote, discountPoints, buyPrice, windowMatched, deltaFromTarget, targetPrice: displayTargetPrice, maxPrice: ladderMaxBuyPrice, priceLadder: buildTargetPriceLadder(requestedRates, getQuoteForRate, quote, quote.purchasePrice, displayPrice, displayTargetPrice, ladderMaxBuyPrice) }; };
