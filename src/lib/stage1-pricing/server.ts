...
function buildTargetPriceLadder(requestedRates: number[], getQuoteForRate: (rateOverride?: number) => Stage1ExecutionQuote, highlightedQuote: Stage1ExecutionQuote): InvestorPriceLadderRow[] {
  ...
  for (const requestedRate of requestedRates) {
    ...
    const pointsDelta = roundToThree(100 - quote.purchasePrice);
