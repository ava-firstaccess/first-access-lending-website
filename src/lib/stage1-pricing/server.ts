...
function buildTargetPriceLadder(getQuoteForTargetPrice: (targetPrice: number) => Stage1ExecutionQuote, maxPrice: number, highlightedTargetPrice: number): InvestorPriceLadderRow[] {
  const rows: InvestorPriceLadderRow[] = [];
  for (let targetPrice = 97; targetPrice <= 103; targetPrice += 1) {
    const cappedTargetPrice = maxPrice > 0 ? Math.min(targetPrice, maxPrice) : targetPrice;
    const quote = getQuoteForTargetPrice(cappedTargetPrice);
    const pointsDelta = roundToThree(cappedTargetPrice - quote.purchasePrice);
    rows.push({
      targetPrice,
      cappedTargetPrice: roundToThree(cappedTargetPrice),
      rate: quote.rate,
      noteRate: quote.noteRate,
      purchasePrice: quote.purchasePrice,
      pointsLabel: pointsDelta >= 0 ? 'Discount' : 'Rebate',
      pointsValue: roundUpToEighth(Math.abs(pointsDelta)),
      highlighted: Math.abs(targetPrice - highlightedTargetPrice) < 0.0001,
    });
  }
  return rows;
}
...
function getActiveResult(engine: PricingViewEngine, input: TesterInput, effectiveTargetPrice: number, effectiveManualRateOverride: number | undefined, tolerance: number, hasTargetPriceOverride: boolean): Stage1PricingEngineResult | null {
  ...
  if (engine === 'Button') {
    ...
    const maxPrice = solveButtonStage1TargetRate(input as ButtonStage1Input, { targetPrice: 999, tolerance, selectedLoanAmount: input.desiredLoanAmount, cesTermYears: input.buttonTermYears, helocDrawTermYears: input.helocDrawTermYears }).purchasePrice;
    const displayQuote = effectiveManualRateOverride !== undefined ? baseQuote : chooseDisplayQuote('Button', toQuote('Button', { ...baseQuote, program: 'Button', product: String(input.buttonProduct || 'HELOC') }), buildRequestedRates(3, 20), rateOverride => toQuote('Button', { ...calculateButtonStage1Quote(input as ButtonStage1Input, { selectedLoanAmount: input.desiredLoanAmount, rateOverride, cesTermYears: input.buttonTermYears, helocDrawTermYears: input.helocDrawTermYears }), program: 'Button', product: String(input.buttonProduct || 'HELOC') }), effectiveTargetPrice, maxPrice, hasTargetPriceOverride);
...
export function computeStage1Pricing(request: Stage1PricingRequest): Stage1PricingResponse {
  ...
  const makeSummary = (eligibility: Stage1Eligibility, quote: Stage1ExecutionQuote, maxPrice: number, getQuoteForTargetPrice: (targetPrice: number) => Stage1ExecutionQuote): InvestorSummary => { const selectionTarget = getBestXSelectionTarget(effectiveTargetPrice, maxPrice, hasTargetPriceOverride); const bestExWindowLowerBound = roundToThree(selectionTarget - BEST_EX_WINDOW_FLOOR); const bestExWindowUpperBound = roundToThree(selectionTarget + BEST_EX_WINDOW_CEILING); const rawDiscountPoints = effectiveTargetPrice - quote.purchasePrice; const discountPoints = roundUpToEighth(Math.abs(rawDiscountPoints)); const buyPrice = 0; const deltaFromTarget = roundToThree(quote.purchasePrice - effectiveTargetPrice); const respectsMaxPrice = maxPrice <= 0 || quote.purchasePrice <= maxPrice + 0.0001; const windowMatched = eligibility.eligible && respectsMaxPrice && quote.purchasePrice >= bestExWindowLowerBound && quote.purchasePrice <= bestExWindowUpperBound; return { investor: quote.engine, eligibility, quote, discountPoints, buyPrice, windowMatched, deltaFromTarget, targetPrice: defaultBackendTargetPrice, maxPrice, priceLadder: buildTargetPriceLadder(getQuoteForTargetPrice, maxPrice, roundToThree(effectiveTargetPrice)) }; };
  ...
  const chooseBestXSummary = (eligibility: Stage1Eligibility, fallbackQuote: Stage1ExecutionQuote, requestedRates: number[], getQuote: (rateOverride?: number) => Stage1ExecutionQuote, getQuoteForTargetPrice: (targetPrice: number) => Stage1ExecutionQuote, maxPrice: number): InvestorSummary => {
    if (!eligibility.eligible) return makeSummary(eligibility, fallbackQuote, maxPrice, getQuoteForTargetPrice);
    ...
    if (effectiveManualRateOverride !== undefined) {
      const manualSummary = makeSummary(eligibility, getQuote(effectiveManualRateOverride), maxPrice, getQuoteForTargetPrice);
      return respectsMax(manualSummary) ? manualSummary : makeSummary(eligibility, fallbackQuote, maxPrice, getQuoteForTargetPrice);
    }
    const candidates = new Map<string, InvestorSummary>();
    for (const requestedRate of requestedRates) {
      const summary = makeSummary(eligibility, getQuote(requestedRate), maxPrice, getQuoteForTargetPrice);
      candidates.set(`${summary.quote.rate}|${summary.quote.purchasePrice}|${summary.quote.product}`, summary);
    }
    const allCandidates = candidates.size ? [...candidates.values()] : [makeSummary(eligibility, fallbackQuote, maxPrice, getQuoteForTargetPrice)];
    ...
    if (windowCandidates.length > 0) {
      return [...windowCandidates].sort((a, b) => a.quote.rate - b.quote.rate || Math.abs(a.deltaFromTarget) - Math.abs(b.deltaFromTarget) || ((a.deltaFromTarget > 0) === (b.deltaFromTarget > 0) ? 0 : a.deltaFromTarget > 0 ? 1 : -1) || b.buyPrice - a.buyPrice || a.investor.localeCompare(b.investor))[0];
    }
  ...
  if (bestExProduct === 'HELOC') {
    ...
    if (!buttonDocType) results.push(makeIneligible('Button', 'Button', 'HELOC', `Button does not support ${bestExDocType} pricing for HELOC.`));
      ...
      results.push(chooseBestXSummary(buttonEligibility, toQuote('Button', { ...buttonBaseQuote, program: 'Button', product: 'HELOC' }), standardRequestedRates, rateOverride => toQuote('Button', { ...calculateButtonStage1Quote(buttonInput as ButtonStage1Input, { selectedLoanAmount, helocDrawTermYears: bestExDrawPeriodYears, rateOverride }), program: 'Button', product: 'HELOC' }), targetPrice => toQuote('Button', { ...calculateButtonStage1Quote(buttonInput as ButtonStage1Input, { selectedLoanAmount, targetPrice, helocDrawTermYears: bestExDrawPeriodYears }), program: 'Button', product: 'HELOC' }), buttonMaxPrice));
    }
    ...
    const osbDocType = mapBestExDocTypeToOsb(bestExDocType, 'HELOC');
    ...
    if (!buttonDocType) results.push(makeIneligible('Button', 'Button', 'CES', `Button does not support ${bestExDocType} pricing for CES.`));
      ...
      results.push(chooseBestXSummary(buttonEligibility, toQuote('Button', { ...buttonBaseQuote, program: 'Button', product: 'CES' }), standardRequestedRates, rateOverride => toQuote('Button', { ...calculateButtonStage1Quote(buttonInput as ButtonStage1Input, { selectedLoanAmount, cesTermYears: bestExTermYears, rateOverride }), program: 'Button', product: 'CES' }), targetPrice => toQuote('Button', { ...calculateButtonStage1Quote(buttonInput as ButtonStage1Input, { selectedLoanAmount, targetPrice, cesTermYears: bestExTermYears }), program: 'Button', product: 'CES' }), buttonMaxPrice));
    }
    ...
    else { const vistaInput = { ...input, vistaProduct, vistaDocType: mapBestExDocTypeToVista(bestExDocType) }; const eligibility = evaluateVistaStage1Eligibility(vistaInput, selectedLoanAmount); const baseQuote = calculateVistaStage1Quote(vistaInput, { selectedLoanAmount, targetPrice: effectiveTargetPrice }); const maxPrice = solveVistaStage1TargetRate(vistaInput, { targetPrice: 999, tolerance, selectedLoanAmount }).purchasePrice; results.push(chooseBestXSummary(eligibility, toQuote('Vista', baseQuote), standardRequestedRates, rateOverride => toQuote('Vista', calculateVistaStage1Quote(vistaInput, { selectedLoanAmount, targetPrice: effectiveTargetPrice, rateOverride })), targetPrice => toQuote('Vista', calculateVistaStage1Quote(vistaInput, { selectedLoanAmount, targetPrice })), maxPrice)); }
    ...
    else { const newrezInput = { ...input, newrezProduct }; const eligibility = evaluateNewRezStage1Eligibility(newrezInput, selectedLoanAmount); const baseQuote = calculateNewRezStage1Quote(newrezInput, { selectedLoanAmount, targetPrice: effectiveTargetPrice }); const maxPrice = solveNewRezStage1TargetRate(newrezInput, { targetPrice: 999, tolerance, selectedLoanAmount }).purchasePrice; results.push(chooseBestXSummary(eligibility, toQuote('NewRez', baseQuote), standardRequestedRates, rateOverride => toQuote('NewRez', calculateNewRezStage1Quote(newrezInput, { selectedLoanAmount, targetPrice: effectiveTargetPrice, rateOverride })), targetPrice => toQuote('NewRez', calculateNewRezStage1Quote(newrezInput, { selectedLoanAmount, targetPrice })), maxPrice)); }
    ...
    const osbDocType = mapBestExDocTypeToOsb(bestExDocType, 'CES');
    ...
    else if (actualLockPeriodDays !== 30 && actualLockPeriodDays !== 45 && actualLockPeriodDays !== 60) results.push(makeIneligible('Verus', 'CES', verusProducts[bestExTermYears]!, `Verus only supports padded lock pricing at 30, 45, or 60 days, not ${actualLockPeriodDays}.`));
