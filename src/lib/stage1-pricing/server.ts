...
function buildTargetPriceLadder(requestedRates: number[], getQuoteForRate: (rateOverride?: number) => Stage1ExecutionQuote, highlightedQuote: Stage1ExecutionQuote, referencePrice: number): InvestorPriceLadderRow[] {
  const rows = new Map<string, InvestorPriceLadderRow>();

  for (const requestedRate of requestedRates) {
    const quote = getQuoteForRate(requestedRate);
    if (quote.purchasePrice < 97 || quote.purchasePrice > 103) continue;

    const pointsDelta = roundToThree(referencePrice - quote.purchasePrice);
  ...
  return [...rows.values()].sort((a, b) => a.purchasePrice - b.purchasePrice || a.rate - b.rate || a.noteRate - b.noteRate);
}
