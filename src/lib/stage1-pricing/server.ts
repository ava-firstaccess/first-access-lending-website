...
function chooseDisplayQuote(engine: Stage1ExecutionQuote['engine'], fallbackQuote: Stage1ExecutionQuote, requestedRates: number[], getQuote: (rateOverride?: number) => Stage1ExecutionQuote, targetPrice: number, maxPrice: number) {
...
export function computeStage1Pricing(request: Stage1PricingRequest): Stage1PricingResponse {
  ...
  const makeSummary = (eligibility: Stage1Eligibility, quote: Stage1ExecutionQuote, maxPrice: number): InvestorSummary => { const cappedSelectionTarget = capSelectionTarget(effectiveTargetPrice, maxPrice); const bestExWindowLowerBound = roundToThree(cappedSelectionTarget - BEST_EX_WINDOW_FLOOR); const bestExWindowUpperBound = roundToThree(cappedSelectionTarget + BEST_EX_WINDOW_CEILING); const rawDiscountPoints = effectiveTargetPrice - quote.purchasePrice; const discountPoints = roundUpToEighth(Math.abs(rawDiscountPoints)); const buyPrice = 0; const deltaFromTarget = roundToThree(quote.purchasePrice - effectiveTargetPrice); const windowMatched = eligibility.eligible && quote.purchasePrice >= bestExWindowLowerBound && quote.purchasePrice <= bestExWindowUpperBound; return { investor: quote.engine, eligibility, quote, discountPoints, buyPrice, windowMatched, deltaFromTarget, targetPrice: defaultBackendTargetPrice, maxPrice }; };
