        ...
        <div className="rounded-[28px] border border-emerald-400/20 bg-white/5 p-7 shadow-xl shadow-slate-950/30 backdrop-blur">
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-300">This card has been pulled back from Supabase-based mirror reporting. A real replacement should come from Clear Capital-side reporting or a vendor order-history endpoint, not our internal analytics tables.</p>
            </div>
            <div className="rounded-xl border border-white/15 bg-slate-950/40 px-4 py-2 text-sm font-semibold text-slate-300">Not wired yet</div>
          </div>

          <div className="mt-6 rounded-3xl border border-amber-400/20 bg-amber-500/10 p-5 text-sm leading-6 text-amber-100">
            <div className="font-semibold text-white">What still needs to happen</div>
            <ul className="mt-3 space-y-2">
              <li>• Find the actual Clear Capital reporting or order-history endpoint for AVMs</li>
              <li>• Or wire the Clear Capital portal/reporting surface if that is the real source</li>
              <li>• Then replace this placeholder with vendor-native results only</li>
            </ul>
          </div>
        </div>
      </div>
