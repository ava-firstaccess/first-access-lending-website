// Finalize Details: Full 1003 data collection form
...
import { isSectionComplete, VisibilityRule } from '@/components/quote/ConditionalEngine';
...
function AnnualMonthlyField({ label, namePrefix, formData, onChange, required = false }: {
  ...
  const initialMode = formData[inputModeKey] === 'annual' ? 'annual' : 'monthly';
  ...
  const rawInput = String(formData[inputKey] ?? formData[monthlyKey] ?? '');
...
function Stage2Content() {
  ...
  useEffect(() => {
    ...
    if (stage1Data.loanBalance && stage1Data.loanBalance !== '0' && Number(stage1Data.loanBalance) > 0) {
      ...
      stage1Data['Current Loan - First Mortgage Balance'] = Number(stage1Data.loanBalance);
    } else {
    ...
    if (!stage1Data['Subject Property - Occupancy'] && stage1Data.propertyType) {
      stage1Data['Subject Property - Occupancy'] = occupancyMap[String(stage1Data.propertyType)] || '';
    }
    ...
    if (stage1Data.structureType && !stage1Data['Subject Property - Structure Type']) {
      ...
      stage1Data['Subject Property - Structure Type'] = structureMap[String(stage1Data.structureType)] || '';
    }
    ...
    const localDataTimer = window.setTimeout(() => {
    ...
    return () => {
      window.clearTimeout(localDataTimer);
    };
  }, []);
  ...
  const sections = {
    ...
    employmentIncome: [
      ...
      ...(hasVariableIncome('Borrower', 'Overtime') ? [
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (data?.applicationId) {
        localStorage.setItem('submittedApplicationId', String(data.applicationId));
      }
        ...
        <div className="grid lg:grid-cols-3 lg:gap-8 gap-6">
          <div className="lg:col-span-2">
            ...
            <div className="mb-8">
              ...
              <h1 className="text-3xl font-bold text-gray-900 mb-2">Finalize Details</h1>
              <p className="text-gray-600">A few final details so we can package everything accurately and keep things moving.</p>
            </div>
